...
public class ImageUploadServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		ServletFileUpload upload = new ServletFileUpload(new DiskFileItemFactory(1024 * 1024, new File(ctx.getInitParameter("tempdir"))));//1M=1024*1024
		ImgUploadVO vo= new ImgUploadVO();
		List<String> imageName= new ArrayList<>();//업로드한 파일명을 저장할 변수.
		...
		try {
			...
			for (FileItem item : list) {
				...
				if (item.isFormField()) {// 일반 요청파라미터(String)//파일이아닌 그냥 text도 이 방법이 더 좋다._getParameter를 사용하면 잘못 전달되는게 있을 수도 있다.
					...
					//UUID : 범용알고리즘. 실행할때마다 중복되지 않는 문자열 생성.
